...
def test_home_contains_tagline():
    """Test that the response body contains the tagline"""
    response = client.get("/")
    assert "Come in. Sit down. Tell us about your human." in response.text


def test_get_complaints_status_code():
    """Test that GET /complaints returns status code 200"""
    response = client.get("/complaints")
    assert response.status_code == 200


def test_get_complaints_contains_seed_complaint():
    """Test that GET /complaints response contains a seed complaint's text"""
    response = client.get("/complaints")
    # Check for part of the complaint text (avoiding apostrophe HTML encoding)
    assert "finish a task, the requirements change" in response.text


def test_post_complaints_returns_redirect():
    """Test that POST /complaints returns a 303 redirect"""
...
def test_post_complaints_adds_complaint():
    """Test that after POST /complaints, a subsequent GET contains the new complaint"""
